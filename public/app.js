...
const toggleItem = (title) => {
    ...
    let itemsTitleArray = [];
    itemsList.forEach((item) => (itemsTitleArray = [...itemsTitleArray, item.title]));
    if (!checkIfArrayIncludes(title, itemsTitleArray)) {
        return;
    }
    console.log(`'${title}' toggled`);
    itemsList.map((item) => item.title === title ? (item.active = !item.active) : item.active);
    saveToJson(itemsList);
};
...
const checkIfArrayIncludes = (item, array) => {
...
let appRunning = true;
while (appRunning) {
    const decision = input("Choose an option: 1=Show 2=Add 3=Toggle active/inactive 4=Delete 0=Exit ");
    switch (decision) {
        case "0":
            appRunning = false;
            break;
        case "1":
            console.log("\n");
            readFile().forEach((item) => item.active
                ? console.log(colors.bgBlack.blue(`${item.title}:`), " ", colors.green("active"))
                : console.log(colors.bgBlack.blue(`${item.title}:`), " ", colors.red("inactive")));
            console.log("\n");
            break;
        case "2":
